...
import { searchEbay, sendImageContext, checkConfig, isGitHubPages } from './services/api';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    checkConfig().then(setIsConfigured).catch(() => {
      // If on GitHub Pages and config check fails, might need backend config
      if (isGitHubPages()) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-8 flex justify-between items-start">
          ...
          <div className="flex gap-2">
            {isGitHubPages() && (
              <button
                onClick={() => setIsBackendConfigOpen(true)}
                className="flex items-center gap-2 px-4 py-2 bg-orange-100 border border-orange-300 rounded-lg hover:bg-orange-200 transition-colors text-orange-700"
      ...
      <BackendConfig
        isOpen={isBackendConfigOpen}
        onClose={() => setIsBackendConfigOpen(false)}
      />
    </div>
  );
};
